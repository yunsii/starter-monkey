...
|                       | `install <serverUrl>`                        | `install-build`          |
| --------------------- | -------------------------------------------- | ------------------------ |
...
**默认用 dev server**，热更新的价值很大。遇到严格 CSP 的站点、或 Chrome 拦住
HTTPS 页面访问 `http://localhost` 时改用 `install-build`——它比装 Disable CSP
扩展 + 改 `chrome://flags` 轻得多，也不需要真人动浏览器。
